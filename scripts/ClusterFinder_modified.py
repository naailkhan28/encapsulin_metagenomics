...
organism_name = 'cargo_clusterfinder'
input_file = 'cargo_clusterfinder_input_data.txt'

# --- read in the HMM model data
PATH_TO_FREQUENCIES = './freqdata/' # link to where the frequencies are
PATH_TO_DATA = '../metagenomics/metadata/'
...
for d in D:
	...
	gene,pfamID = d[1],d[6]
	genstart,genestop = int(d[2]),int(d[3])
	try: pfamstart,pfamstop = int(d[4]),int(d[5])
